...
function renderTree(config, items, searchTerm = "") {
  ...
  fx.go(
    items,
    fx.each((item) => {
      const nodeEl = createNodeElement.call(
        this,
        config,
        item,
        normalizedSearch,
...
function createNodeElement(config, item, searchTerm, depth) {
  ...
  const hasMatchingChildren =
    hasChildren && hasMatchingDescendants(children, fields, searchTerm);
  ...
  if (hasChildren) {
    ...
    fx.go(
      children,
      fx.each((child) => {
        const childEl = createNodeElement.call(
          this,
          config,
          child,
          searchTerm,
...
function hasMatchingDescendants(children, fields, searchTerm) {
  ...
  return children.some((child) => {
    ...
    return (
      ...
      hasMatchingDescendants(grandChildren, fields, searchTerm)
    );
  });
}
...
function setupInternalHandlers() {
  ...
  root
    ...
    ?.addEventListener("click", this._internalHandlers.expandAllClick);
  root
    .querySelector(".btn-collapse-all")
    ?.addEventListener("click", this._internalHandlers.collapseAllClick);
  root
    .querySelector(".search-input")
    ?.addEventListener("input", this._internalHandlers.searchInput);
}
...
function selectNode(nodeId) {
  const prevSelected = this.appendElement.querySelector(
  ...
  const nodeEl = this.appendElement.querySelector(
